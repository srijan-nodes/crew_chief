            ...
            foreach (var dub in outputDuplications)
            {
                ...
                for (; !captureDone && captureRetry < MAX_CAPTURE_RETRY_COUNT; captureRetry++)
                {
                    ...
                    catch (SharpDXException e)
                    {
                        ...
                        {
                            OnDeviceAccessLost();
                            Log.Exception(e, "CaptureScreen.Capture: device access lost = ");
                        ...
                        {
                            Log.Exception(e, "CaptureScreen.Capture: screen capturing failed = ");
                        }
                    }
                    catch (Exception ex)
                    {
...
#if DEBUG
                    Log.Commentary(Log.DontSpam("CaptureScreen.Capture: Capture retry count exceeded.",10));
#endif
