    ...
    public static class ThreadManager
    {
        ...
        public static void UnregisterTemporaryThread(Thread t, bool killIfAlive = false)
        {
            ...
            lock (ThreadManager.temporaryThreadsLock)
            {
                if (ThreadManager.temporaryThreads.Contains(t))
                {
                    ...
                    else if (!UnitTest.UnitTest.Active)
                    {
                        // This is not necessarily a problem, but this message is here to make thread author think about spammy threads.
        ...
        private static bool WaitForRootThreadsStart(CrewChief cc)
        {
            try
            {
                ...
                ThreadManager.Trace("Waiting for root threads to start...");
                for (int i = 0; i < ThreadManager.THREAD_ALIVE_WAIT_ITERATIONS; ++i)
                {
                    ...
                    if (allThreadsRunning)
                    {
                        ...
                        if (isTraceFileSet)
                        {
                            ThreadManager.Trace("Waiting for run thread to read data file...");
                            while (true)
                            {
        ...
        private static bool WaitForRootAndTemporaryThreadsStop(CrewChief cc)
        {
            try
            {
                ...
                if (CrewChief.Debug.UserTraceLogging)
                {
                    ThreadManager.Trace("Waiting for run thread to dump data file...");
                    while (true)
                    {
        ...
        private static bool ThreadWaitHelper(
            ...
            bool isShutdown)
        {
            ThreadManager.Trace((isShutdown ? "Shutdown - " : "") + "Waiting for " + threadSetName + " threads to stop...");
