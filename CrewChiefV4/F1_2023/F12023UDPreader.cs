...
{
    public class F12023UDPreader : GameDataReader
    {
        ...
        private List<F12023StructWrapper> dataToDump;
        private F12023StructWrapper workingData = new F12023StructWrapper();
        private F12023StructWrapper[] dataReadFromFile = null;
        private int dataReadFromFileIndex = 0;
        private readonly int udpPort = UserSettings.GetUserSettings().getInt("f1_2023_udp_data_port");
        ...
        public F12023UDPreader()
        {
            dataTransformByPacketId = new Dictionary<PacketId, Action<GCHandle>>()
            {
                ...
                { PacketId.TyreSets, TransformTyreSetsData },
                { PacketId.MotionEx, TransformMotionEx }
            };
        }
        ...
        public override void DumpRawGameData()
        {
            ...
            SerializeObject(dataToDump.ToArray<F12023StructWrapper>(), filenameToDump);
        }
        ...
        public override TracedGameData ReadGameDataFromFile(string filename, int pauseBeforeStart)
        {
            ...
            if (shouldReadNewFile)
            {
                ...
                dataReadFromFile = DeSerializeObject<F12023StructWrapper[]>(filePathResolved);
            ...
            F12023StructWrapper structWrapperData = dataReadFromFile[dataReadFromFileIndex];
        ...
        {
            F12023StructWrapper latestData = workingData.CreateCopy(DateTime.UtcNow.Ticks, forSpotter);
        ...
        {
            // Sixth byte is always the packet id
            var packetId = (PacketId)rawData[6];
        ...
        private void Init()
        {
            ...
            if (dumpToFile) dataToDump = new List<F12023StructWrapper>();
        ...
        private void TransformTyreSetsData(GCHandle handle)
        {
            workingData.packetTyreSetsData = handle.PointerCast<PacketTyreSetsData>();
        }

        private void TransformMotionEx(GCHandle handle)
        {
            workingData.packetMotionExData = handle.PointerCast<PacketMotionExData>();
        }
