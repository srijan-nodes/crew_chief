...
namespace DarkModeForms
{
	...
	public static class Messenger
	{
			...
			MessageBoxDefaultButton defaultButton = MessageBoxDefaultButton.Button1, Form owner = null)
		{
			...
			{
				picIcon = new PictureBox { SizeMode = PictureBoxSizeMode.Zoom, Size = new Size(64, 64) };
			...
			MsgIcon Icon = 0, MessageBoxButtons buttons = MessageBoxButtons.OK, bool pIsDarkMode = true)
		{
			...
			if (Icon != MsgIcon.None)
			{
				picIcon = new PictureBox { SizeMode = PictureBoxSizeMode.Zoom, Size = new Size(48, 48) };
				picIcon.Image = _Icons.GetIcon(Icon);
		...
		{
			public Timer Timer;
			public EventHandler DisposeHandler;
		}
		private static Dictionary<Control, TimerInfo> timers;
		...
		private static void AddTextChangedDelay<TControl>(TControl control, int milliseconds, Action<TControl> action) where TControl : Control
		{
			if (timers == null)
			{
				timers = new Dictionary<Control, TimerInfo>();
			}

			if (timers.TryGetValue(control, out TimerInfo oldInfo))
			{
				control.Disposed -= oldInfo.DisposeHandler;
				oldInfo.Timer.Stop();
				oldInfo.Timer.Dispose();
				timers.Remove(control);
			}

			var info = new TimerInfo();
			info.Timer = new Timer();
			info.Timer.Interval = milliseconds;

			info.DisposeHandler = (s, e) =>
			{
				control.Disposed -= info.DisposeHandler;
				if (timers.TryGetValue(control, out TimerInfo ct) && ct == info)
				{
					ct.Timer.Stop();
					ct.Timer.Dispose();
					timers.Remove(control);
