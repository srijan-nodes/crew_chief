...
namespace DarkModeForms
{
    ...
    public class DarkModeCS
    {
        ...
        /// </summary>
        public static bool IsDarkModeCSEnabled { get; set; } = false;
        ...
        public DarkModeCS(Form _Form, bool _ColorizeIcons = true, bool _RoundedPanels = false)
        {
            ...
            if (originalWndProc == IntPtr.Zero)
            {
                ...
                _Form.HandleDestroyed += (sender, e) =>
                {
                    if (originalWndProc != IntPtr.Zero)
                    {
                        HandleRef handleRef = new HandleRef(_Form, _Form.Handle);
                        SetWindowLongPtr(handleRef, GWLP_WNDPROC, originalWndProc);
                        originalWndProc = IntPtr.Zero;
                    }
                    if (ownerFormControlAdded != null)
                        _Form.ControlAdded -= ownerFormControlAdded;
                    UnhookThemeHandlers(_Form);
                };
            }
        ...
        private void UnhookThemeHandlers(Control control)
        {
                ...
                control.Paint += (sender, e) =>
                {
                    if (control.Enabled == false && IsDarkMode)
                    {
                        ...
                        using (Brush B = new SolidBrush(control.ForeColor))
                        {
                            ...
                            if (mi != null)
                            {
                                using (StringFormat sf = mi.Invoke(lbl, new object[] { }) as StringFormat)
                                {
        ...
        public static void SetRoundBorders(Control _Control, int Radius = 10, Color? borderColor = null, int borderSize = 2, bool underlinedStyle = false)
        {
            ...
            if (_Control != null)
            {
                ...
                IntPtr hRgn = CreateRoundRectRgn(0, 0, _Control.Width, _Control.Height, Radius, Radius);
                var initialOldReg = _Control.Region;
                _Control.Region = Region.FromHrgn(hRgn);
                initialOldReg?.Dispose();
                DeleteObject(hRgn);
