    ...
    public partial class ListPropertyControl : UserControl
    {
        ...
        private void checkDefault()
        {
            try
            {
                if (this.filter != null)
                {
                    this.filter.propertyNotDefault = this.availableValues[this.comboBox1.SelectedIndex] != this.defaultValue;
                }
            }
            catch (Exception e)
            {
                Log.Error($"{Configuration.uiTextFilePathForErrorReporting} does not have 'listprop_value_0' etc. for '{propertyId}'");
            }
        }
