...
namespace CrewChiefV4
{
    ...
    {
        private const float calculateOpponentSpeedsEvery = 0.2f;

        private const float carBehindExtraLength = 0.4f; // Gives earlier warning of a car behind?
        ...
        private const int clearMessageExpiresAfter = 2000;
        private const int clearAllRoundMessageExpiresAfter = 2000;
        private const int holdMessageExpiresAfter = 1000;
        private const int inTheMiddleMessageExpiresAfter = 1000;
        ...
        private readonly float gapNeededForClear = UserSettings.GetUserSettings().getFloat("spotter_gap_for_clear");

        private float longCarLength;
        
        // don't play spotter messages if we're going < 10ms
        private readonly float minSpeedForSpotterToOperate = UserSettings.GetUserSettings().getFloat("min_speed_for_spotter");
        ...
        private readonly TimeSpan repeatHoldFrequency = TimeSpan.FromSeconds(UserSettings.GetUserSettings().getInt("spotter_hold_repeat_frequency"));
        
        // say "still there" every 5 (?) seconds on ovals
        private readonly TimeSpan ovalRepeatHoldFrequency = TimeSpan.FromSeconds(UserSettings.GetUserSettings().getInt("spotter_hold_repeat_frequency_ovals"));

        // use half the 'still there' wait if we're bouncing between clear and overlap
        private readonly TimeSpan bouncingWait = TimeSpan.FromSeconds(UserSettings.GetUserSettings().getInt("spotter_hold_repeat_frequency") / 2);

        // this is the delay between saying "car left" then "3 wide, you're on the right"
        private readonly TimeSpan onSingleOverlapTo3WideDelay = TimeSpan.FromSeconds(0.5);

        private int carsOnLeftAtPreviousTick;
        private int carsOnRightAtPreviousTick;

        private const float trackZoneToConsider = 20f;
        private float carWidth;
        private readonly float maxClosingSpeed = UserSettings.GetUserSettings().getFloat("max_closing_speed_for_spotter");

        // Default folders, updated if a custom spotter is selected
        ...
        static readonly List<string> allImmediateMessages = new List<string>
        {
            folderStillThere,
            folderInTheMiddle,
            folderCarLeft,
            ...
            folderClearOutside,
            folderClearAllRound,
            folderThreeWideYoureOnRight,
            folderThreeWideYoureOnLeft,
            folderThreeWideYoureOnInside,
            folderThreeWideYoureOnOutside
        };
        ...
        private readonly Boolean use3WideLeftAndRight = UserSettings.GetUserSettings().getBoolean("spotter_enable_three_wide_left_and_right");
        ...
        /// <summary>
        /// Calculate if an opponent (or opponents) is close to overlapping the player, and if so, play the appropriate message.
        /// </summary>
        /// <param name="opponentVelocityData">optional: It should be a list of 2 element arrays with x and z speed, ordered the same as currentOpponentPositions</param>
        /// <param name="opponentSpeedData">optional: It should be an array with z speed</param>
        public void triggerInternal(float playerRotationInRadians,
            float[] currentPlayerPosition,
            float[] playerVelocityData,
            List<float[]> currentOpponentPositions,
            List<float[]> opponentVelocityData = null,
            float[] opponentSpeedData = null)
        {
        ...
        private Tuple<Side, float> getSideAndSeparation(float playerRotationInRadians, float playerX, float playerZ, float oppponentX, float opponentZ, Boolean isOpponentSpeedInRange)
        {
            ...
            if (Math.Abs(alignedXCoordinate) < trackZoneToConsider)
            {
                ...
                { // car on the right
                    ...
                    else if (((alignedZCoordinate < 0 && alignedZCoordinate * -1 < carLength) || 
                              (alignedZCoordinate > 0 && alignedZCoordinate < carLength + carBehindExtraLength)) &&
                    ...
                    // alignedZCoordinate won't be 0 because the car would have already been overlapping
                }
                else
                { // ditto for car on the left
                ...
                if (messageIsValid(nextMessageType, carsOnLeftCount, carsOnRightCount))
                {
                    ...
                    switch (nextMessageType)
                    {
                        case NextMessageType.threeWide:
                            removeAllImmediateMessagesExcept(new List<string>
                            {
                                folderInTheMiddle
                            });
                        ...
                        case NextMessageType.clearAllRound:
                            ...
                            {
                                QueuedMessage clearAllRoundMessage = new QueuedMessage(folderClearAllRound, clearAllRoundMessageExpiresAfter);
                                removeAllImmediateMessagesExcept(new List<string>
                                {
                                    folderClearAllRound
                                });
                        ...
                        case NextMessageType.clearLeft:
                            if (reportedSingleOverlapLeft || reportedDoubleOverlapLeft)
                            {
                                ...
                                {
                                    QueuedMessage clearAllRoundMessage = new QueuedMessage(folderClearAllRound, clearMessageExpiresAfter);
                                    removeAllImmediateMessagesExcept(new List<string>
                                    {
                                        folderClearAllRound
                                    });
                                ...
                                else
                                {
                                    ...
                                    QueuedMessage clearLeftMessage = new QueuedMessage(selectedMessage, clearMessageExpiresAfter);
                                    removeAllImmediateMessagesExcept(new List<string>
                                    {
                                        folderClearInside,
                                        folderClearLeft
                        ...
                        case NextMessageType.clearRight:
                            if (reportedSingleOverlapRight || reportedDoubleOverlapRight)
                            {
                                ...
                                else
                                {
                                    ...
                                    QueuedMessage clearRightMessage = new QueuedMessage(selectedMessage, clearMessageExpiresAfter);
                                    removeAllImmediateMessagesExcept(new List<string>
                                    {
                                        folderClearOutside, 
                                        folderClearRight
                        ...
                        case NextMessageType.stillThere:
                            if (reportedSingleOverlapLeft || reportedSingleOverlapRight || reportedDoubleOverlapLeft || reportedDoubleOverlapRight)
                            {
                                ...
                                audioPlayer.removeImmediateMessages(new List<String>
                                {
                                    folderClearRight,
                                    folderClearLeft,
                                    folderClearInside,
        ...
        private void removeAllImmediateMessagesExcept(List<string> immediateMessagesToKeep)
        {
            // Create a new list excluding the messages to keep
            var messagesToRemove = allImmediateMessages
                .Where(msg => !immediateMessagesToKeep.Contains(msg))
                .ToList();
            // Remove the messages we don't want 
            audioPlayer.removeImmediateMessages(messagesToRemove);
        }
    }
}
