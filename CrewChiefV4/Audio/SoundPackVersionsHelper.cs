...
namespace CrewChiefV4.Audio
{
    class SoundPackVersionsHelper
    {
        ...
        public static String retryReplaceGit;
        public static String retryReplaceWithGit;
        ...
        public static Boolean parseUpdateData(String updateXML, String languageToCheck)
        {
            try
            {
                ...
                if (doc.Descendants("sounds").Count() > 0)
                {
                    ...
                    try
                    {
                        retryReplaceGit = doc.Descendants("retry_replace_git").First().Value;
                        retryReplaceWithGit = doc.Descendants("retry_replace_with_git").First().Value;
                    }
                    catch (Exception)
                    {
                        Console.WriteLine("No Git retry download location available");
                    }
        ...
        public static string replaceUrlToRetry(string downloadUrl)
        {
            try
            {
                if (downloadUrl == null)
                {
                    Log.Error("Soundpack URL is null");
                    return null;
                }
                if (downloadUrl.Contains(SoundPackVersionsHelper.retryReplaceGit))
                {
                    downloadUrl = downloadUrl.Replace(SoundPackVersionsHelper.retryReplaceGit, SoundPackVersionsHelper.retryReplaceWithGit);
                    // + "?ref_type=heads&inline=false";
                }
                else
                {
                    downloadUrl = downloadUrl.Replace(SoundPackVersionsHelper.retryReplace, SoundPackVersionsHelper.retryReplaceWith);
                }
            }
            catch (Exception e)
            {
                Log.Exception(e, $"Error replacing URL to retry in '{downloadUrl}'");
            }
            return downloadUrl;
        }
