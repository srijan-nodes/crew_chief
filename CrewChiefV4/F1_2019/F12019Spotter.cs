...
{
    class F12019Spotter : Spotter
    {
        private float twoPi = (float)(2 * Math.PI);

        public F12019Spotter(AudioPlayer audioPlayer, Boolean initialEnabledState)
        {
            this.audioPlayer = audioPlayer;
            this.enabled = initialEnabledState;
            this.initialEnabledState = initialEnabledState;
            this.internalSpotter = new NoisyCartesianCoordinateSpotter(audioPlayer, true, UserSettings.GetUserSettings().getFloat("f1_2019_spotter_car_length"), 2);
        }
        ...
        {
            F12019StructWrapper currentWrapper = (F12019StructWrapper)currentStateObj;
