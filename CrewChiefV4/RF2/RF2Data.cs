...
namespace rF2SharedMemory
{
    ...
    public class rFactor2Constants
    {
        ...
        public const int MM_TELEMETRY_STRUCT_SIZE = 241680;
        public const int MM_SCORING_STRUCT_SIZE = 75312;
        public const int MM_RULES_STRUCT_SIZE = 45272;
        public const int MM_EXTENDED_STRUCT_SIZE = 10152;
        ...
        public enum IP_VehicleChampionship
        {
            WEC_2023 = 0x00,
            WEC_2024,
            WEC_2025, 
    ...
    namespace rFactor2Data
    {
        ...
        public struct rF2VehicleTelemetry
        {
            ...
            [JsonIgnore] public byte mVehicleClass;                                   // IP_VehicleClass
            [JsonIgnore] public byte mVehicleChampionship;                            // IP_VehicleChampionship
            #endregion LMU-specific
            // Future use
        ...
        public struct rF2ScoringInfo
        {
            ...
            #region LMU-specific
            [JsonIgnore] public float mSessionTimeRemaining;
            [JsonIgnore] public float mTimeOfDay;
            [JsonIgnore] public byte mIsFixedSetup;
            ...
            // 3 = high (heavy)
            // 4 = saturated
            [JsonIgnore] public byte mTrackGripLevel;
            // Sky type:
            // 0 = clear
            // 1 = light clouds
            // 2 = partially cloudy
            ...
            // 10 = overcast & storm
            [JsonIgnore] public byte mCloudCoverage;
            [JsonIgnore] public byte mTrackLimitsStepsPerPenalty;
            [JsonIgnore] public byte mTrackLimitsStepsPerPoint;
            #endregion

                                                                                      // Future use
            [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = 187)]
        ...
        public enum lmuTrackRulesColumn
        {
            LeftLane = 0,                                                             // left (inside)
            MiddleLane,                                                               // middle
            RightLane,                                                                // right (outside)
        ...
        public struct rF2WeatherControlInfo
        {
            ...
            [JsonIgnore] public double[] mRaining;                                    // rain (0.0-1.0) at different nodes

            [JsonIgnore] public double mCloudiness;                                   // general cloudiness (0.0=clear to 1.0=dark), will be automatically overridden to help ensure clouds exist over rainy areas
            [JsonIgnore] public double mAmbientTempK;                                 // ambient temperature (Kelvin)
            [JsonIgnore] public double mWindMaxSpeed;                                 // maximum speed of wind (ground speed, but it affects how fast the clouds move, too)

            [JsonIgnore] public bool mApplyCloudinessInstantly;                       // preferably we roll the new clouds in, but you can instantly change them now
            [JsonIgnore] public bool mUnused1;                                        //
            [JsonIgnore] public bool mUnused2;                                        //
            [JsonIgnore] public bool mUnused3;                                        //

            [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = 508)]
            [JsonIgnore] public byte[] mExpansion;                                    // future use (humidity, pressure, air density, etc.)
        }
        ...
        public struct rF2ForceFeedback
        {
            ...
            [JsonIgnore] public double mForceValue;                                   // Current FFB value reported via InternalsPlugin::ForceFeedback.
        }
        ...
        {
            [JsonIgnore] public rF2Vec3 mCamPos;                                      // camera position

            [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = 3)]
            [JsonIgnore] public rF2Vec3[] mCamOri;                                    // rows of orientation matrix (use TelemQuat conversions if desired), also converts local

            [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = 8)]
            [JsonIgnore] public byte[] mHWND;                                         // app handle

            [JsonIgnore] public double mAmbientRed;
            [JsonIgnore] public double mAmbientGreen;
            [JsonIgnore] public double mAmbientBlue;

            [JsonIgnore] public int mID;                                              // slot ID being viewed (-1 if invalid)
            ...
            [JsonIgnore] public int mCameraType;                                      // see above comments for possible values

            [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = 128)]
            [JsonIgnore] public byte[] mExpansion;                                    // for future use (possibly camera name)
        };
        ...
        public struct rF2Graphics
        {
            ...
            [JsonIgnore] public rF2GraphicsInfo mGraphicsInfo;
        }
        ...
        struct rF2Weather
        {
            ...
            [JsonIgnore] public double mTrackNodeSize;
            [JsonIgnore] public rF2WeatherControlInfo mWeatherInfo;
        }
        ...
        public struct rF2Extended
        {
            ...
            [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = 128)]
            [JsonIgnore] public byte[] mDisplayedMessageUpdateCapture;
        ...
        struct rF2WeatherControl
        {
            ...
            [JsonIgnore] public int mLayoutVersion;
