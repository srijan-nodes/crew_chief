...
namespace CrewChiefV4.AMS2
{
    class AMS2Spotter : Spotter
    {
        ...
        private readonly Boolean enableSpotterInTimetrial = UserSettings.GetUserSettings().getBoolean("enable_spotter_in_timetrial");
        ...
        private readonly float carLength = UserSettings.GetUserSettings().getFloat("ams2_spotter_car_length");
        ...
        public AMS2Spotter(AudioPlayer audioPlayer, Boolean initialEnabledState)
        {
            this.audioPlayer = audioPlayer;
            this.enabled = initialEnabledState;
            this.initialEnabledState = initialEnabledState;
            if (Game.AMS2_NETWORK)
            {
                this.internalSpotter = new NoisyCartesianCoordinateSpotter(audioPlayer, initialEnabledState, carLength, udamWidth);
            }
            else
            {
        ...
        {
            CrewChiefV4.AMS2.AMS2SharedMemoryReader.AMS2StructWrapper currentWrapper = (CrewChiefV4.AMS2.AMS2SharedMemoryReader.AMS2StructWrapper)currentStateObj;
            ams2APIStruct latestRawData = currentWrapper.data;
            int playerIndex = AMS2GameStateMapper.getPlayerIndex(latestRawData);
            ams2APIParticipantStruct playerData = latestRawData.mParticipantData[playerIndex];
        ...
        {
            ams2APIStruct latestRawData = (ams2APIStruct)currentStateObj;
            foreach (ams2APIParticipantStruct participant in latestRawData.mParticipantData)
            {
        ...
        public override void trigger(Object lastStateObj, Object currentStateObj, GameStateData currentGameState)
        {
            ...
            CrewChiefV4.AMS2.AMS2SharedMemoryReader.AMS2StructWrapper currentWrapper = (CrewChiefV4.AMS2.AMS2SharedMemoryReader.AMS2StructWrapper)currentStateObj;
            ams2APIStruct currentState = currentWrapper.data;
            ...
            if (pitMode != ePitMode.PIT_MODE_NONE || 
                ...
                (gameState == eGameState.GAME_INGAME_PAUSED && !CrewChief.Debug.RunningUnderDebugger) ||
            ...
            CrewChiefV4.AMS2.AMS2SharedMemoryReader.AMS2StructWrapper previousWrapper = (CrewChiefV4.AMS2.AMS2SharedMemoryReader.AMS2StructWrapper)lastStateObj;
            ams2APIStruct lastState = previousWrapper.data;
            ...
            {
                int playerIndex = AMS2GameStateMapper.getPlayerIndex(currentState);
                if (playerIndex != currentState.mViewedParticipantIndex)
                {
                    return;
                }
                ams2APIParticipantStruct playerData = currentState.mParticipantData[playerIndex];
                ...
                for (int i = 0; i < currentState.mParticipantData.Count(); i++)
                {
                    ...
                    ams2APIParticipantStruct opponentData = currentState.mParticipantData[i];
