...
{
    class LapTimes_legacy : AbstractEvent
    {
        ...
        public LapTimes_legacy(AudioPlayer audioPlayer)
        {
        ...
        protected override void triggerInternal(GameStateData previousGameState, GameStateData currentGameState)
        {
            ...
            if (currentGameState.SessionData.IsNewLap)
            {
                ...
                if (currentGameState.SessionData.CompletedLaps > 0)
                {
                    ...
                    {
                        Console.WriteLine("lgcy: Laptime: " + TimeSpan.FromSeconds(currentGameState.SessionData.LapTimePrevious).ToString(@"mm\:ss\.fff") + ",  Valid = " + currentGameState.SessionData.PreviousLapWasValid);
                    }
                    else
                    {
                        Console.WriteLine("lgcy: Laptime: " + currentGameState.SessionData.LapTimePrevious.ToString("0.000") + ",  Valid = " + currentGameState.SessionData.PreviousLapWasValid);
                    }
                }
                        ...
                        if (lapIsValid && !currentGameState.PitData.InPitlane)
                        {
                                 ...
                                 || currentGameState.SessionData.SessionType == SessionType.LonePractice))
                            {
                                ...
                                else
                                {
                                    ...
                                    // only play qual / prac deltas for Raceroom as the PCars data is inaccurate for sessions joined part way through
                                    if (frequencyOfPlayerQualAndPracLapTimeReports > Utilities.random.NextDouble() * 10 &&
                                        (!disablePCarspracAndQualPoleDeltaReports || Game.RACE_ROOM ||
                                        Game.IRACING ||
                                        Game.ASSETTO1) &&
                                    ...
                                    lapTimesWindow.Count >= lapTimesWindowSize)
                                {
                                    ...
                                    switch (consistency)
                                    {
                                        ...
                                        case ConsistencyResult.WORSENING:
                                        {
                                            ...
                                            {
                                                Console.WriteLine("lgcy: Skipping 'worsening' laptimes message - inconsistent with lap rating");
                                            }
        ...
        private ConsistencyResult checkAgainstPreviousLaps(Boolean isOval)
        {
            ...
            for (int index = 0; index < lapTimesWindowSize - 1; index++)
            {
                ...
                {
                    Console.WriteLine("lgcy: No data for consistency check");
                    lastConsistencyMessage = ConsistencyResult.NOT_APPLICABLE;
                    return ConsistencyResult.NOT_APPLICABLE;
                }
        ...
        public static List<MessageFragment> getSingleSectorDeltaMessages(SectorSet sector, float playerTime, float comparisonTime, bool selfPace)
        {
            ...
            if (messages.Count > 0)
            {
                Console.WriteLine("lgcy: Sector = " + sector + " delta (-ve = player faster) = " + (playerTime - comparisonTime).ToString("0.000"));
                Console.WriteLine("lgcy: Resolved delta message: " + String.Join(", ", messages));
            }
            else
            {
                Console.WriteLine("lgcy: Skipping sector pace delta message:  Sector = " + sector + " delta (-ve = player faster) = " + (playerTime - comparisonTime).ToString("0.000"));
            }
            return messages;
        }
            ...
            float comparisonSector2, float playerSector3, float comparisonSector3, Boolean comparisonIncludesAllLaps, bool selfPace)
        {
            ...
            if (messageFragments.Count > 0)
            {
                Console.WriteLine("lgcy: Player best sectors " + playerSector1.ToString("0.000") + ",    " + playerSector2.ToString("0.000") + ",    " + playerSector3.ToString("0.000"));
                Console.WriteLine("lgcy: Opponent best sectors " + comparisonSector1.ToString("0.000") + ",    " + comparisonSector2.ToString("0.000") + ",    " + comparisonSector3.ToString("0.000"));
                Console.WriteLine("lgcy: S1 delta (-ve = player faster) = " + (playerSector1 - comparisonSector1).ToString("0.000") +
                    "    S2 delta  = " + (playerSector2 - comparisonSector2).ToString("0.000") +
                    "    S3 delta  = " + (playerSector3 - comparisonSector3).ToString("0.000"));
                Console.WriteLine("lgcy: Resolved delta message: " + String.Join(", ", messageFragments));
            }
