...
namespace CrewChiefV4.Events
{
    class Battery : AbstractEvent
    {
        ...
        public const string folderPercent = "battery/percent";
        ...
        {
            if (!GlobalBehaviourSettings.enabledMessageTypes.Contains(MessageTypes.BATTERY) || Game.PCARS3 /* no fuel useage in pCars3*/)
                return;
        ...
        public static List<SpeechCommands.ID> Commands = new List<SpeechCommands.ID>
        {
            SpeechCommands.ID.CAR_STATUS,
            SpeechCommands.ID.HOWS_MY_BATTERY,
        ...
        public override void respond(String voiceMessage, SpeechCommands.ID cmd)
        {
            switch (cmd)
            {
                case SpeechCommands.ID.HOWS_MY_BATTERY:
                    if (!GlobalBehaviourSettings.enabledMessageTypes.Contains(MessageTypes.BATTERY))
                    {
                        this.audioPlayer.playMessageImmediately(new QueuedMessage(AudioPlayer.folderNoData, 0));
                ...
                case SpeechCommands.ID.STATUS:
                    if (GlobalBehaviourSettings.enabledMessageTypes.Contains(MessageTypes.BATTERY))
                    {
                        this.reportBatteryStatus(cmd == SpeechCommands.ID.HOWS_MY_BATTERY);
                    }
                    break;
            }
        }
        ...
        {
            SpeechCommands.ID cmd = HandlesEvent(voiceMessage);
            if (!GlobalBehaviourSettings.enabledMessageTypes.Contains(MessageTypes.BATTERY))
            {
                this.audioPlayer.playMessageImmediately(new QueuedMessage(AudioPlayer.folderNoData, 0));
            ...
            switch (cmd)
            {
                ...
                case SpeechCommands.ID.CAR_STATUS:
                case SpeechCommands.ID.STATUS:
                    this.reportExtendedBatteryStatus(cmd == SpeechCommands.ID.HOWS_MY_BATTERY, requestedExplicitly);
                    break;
            }
        }
        ...
        public void reportExtendedBatteryStatus(Boolean allowNoDataMessage, Boolean requestedExplicitly)
        {
            ...
            switch (batteryAdvice)
            {
                case BatteryAdvice.BatteryUseSpotOn:
                    ...
                    break;
                case BatteryAdvice.IncreaseBatteryUse:
                    ...
                    break;
                case BatteryAdvice.ReduceBatteryUse:
                    ...
                    break;
                case BatteryAdvice.WontMakeItWithoutPitting:
