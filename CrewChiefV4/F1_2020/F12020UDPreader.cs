...
{
    public class F12020UDPreader : GameDataReader
    {
        ...
        private List<F12020StructWrapper> dataToDump;
        private F12020StructWrapper workingData = new F12020StructWrapper();
        private F12020StructWrapper[] dataReadFromFile = null;
        private int dataReadFromFileIndex = 0;
        private readonly int udpPort = UserSettings.GetUserSettings().getInt("f1_2020_udp_data_port");
        ...
        public override void DumpRawGameData()
        {
            ...
            {
                SerializeObject(dataToDump.ToArray<F12020StructWrapper>(), filenameToDump);
            }
        }
        ...
        public override TracedGameData ReadGameDataFromFile(String filename, int pauseBeforeStart)
        {
            if (dataReadFromFile == null || filename != lastReadFileName)
            {
                ...
                dataReadFromFile = DeSerializeObject<F12020StructWrapper[]>(filePathResolved);
                lastReadFileName = filename;
                Thread.Sleep(pauseBeforeStart);
            }
            if (dataReadFromFile != null && dataReadFromFile.Length > dataReadFromFileIndex)
            {
                F12020StructWrapper structWrapperData = dataReadFromFile[dataReadFromFileIndex];
        ...
        protected override Boolean InitialiseInternal()
        {
            if (!this.initialised)
            {
                ...
                if (dumpToFile)
                {
                    dataToDump = new List<F12020StructWrapper>();
                }
        ...
        {
            F12020StructWrapper latestData = workingData.CreateCopy(DateTime.UtcNow.Ticks, forSpotter);
            lock (this)
            {
                if (!initialised)
                {
