...
{
    class F12020Spotter : Spotter
    {
        private float twoPi = (float)(2 * Math.PI);

        public F12020Spotter(AudioPlayer audioPlayer, Boolean initialEnabledState)
        {
            this.audioPlayer = audioPlayer;
            this.enabled = initialEnabledState;
            this.initialEnabledState = initialEnabledState;
            this.internalSpotter = new NoisyCartesianCoordinateSpotter(audioPlayer, true, UserSettings.GetUserSettings().getFloat("f1_2020_spotter_car_length"), 2);
        }
        ...
        {
            F12020StructWrapper currentWrapper = (F12020StructWrapper)currentStateObj;
