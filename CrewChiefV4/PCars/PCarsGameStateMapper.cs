...
namespace CrewChiefV4.PCars
{
    class PCarsGameStateMapper : GameStateMapper
    {
        ...
        private readonly Boolean attemptPitDetection = UserSettings.GetUserSettings().getBoolean("attempt_pcars_opponent_pit_detection");
        private readonly Boolean enablePCarsPitWindowStuff = UserSettings.GetUserSettings().getBoolean("enable_pcars_pit_window_messages");
        private static readonly string userSpecifiedSteamId = UserSettings.GetUserSettings().getString("pcars_steam_id");
                            ...
                            if (participantStruct.mIsActive)
                            {
                                ...
                                if (previousGameState != null)
                                {
                                    ...
                                    currentOpponentData.DeltaTime.SetNextDeltaPoint(currentOpponentLapDistance, currentOpponentData.CompletedLaps,
                                        currentOpponentData.Speed, currentGameState.Now, !currentOpponentData.InPits);
                                    if (previousOpponentData != null)
                                    {
