...
{
    class F12021Spotter : Spotter
    {
        private float twoPi = (float)(2 * Math.PI);

        public F12021Spotter(AudioPlayer audioPlayer, Boolean initialEnabledState)
        {
            this.audioPlayer = audioPlayer;
            this.enabled = initialEnabledState;
            this.initialEnabledState = initialEnabledState;
            this.internalSpotter = new NoisyCartesianCoordinateSpotter(audioPlayer, true, UserSettings.GetUserSettings().getFloat("f1_2021_spotter_car_length"), 2);
        }
        ...
        {
            F12021StructWrapper currentWrapper = (F12021StructWrapper)currentStateObj;
