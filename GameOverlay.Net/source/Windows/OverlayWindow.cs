...
namespace GameOverlay.Windows
{
    ...
    public class OverlayWindow : IDisposable
    {
        ...
        private volatile bool _isAppWindow;
        ...
        private volatile IntPtr _windowInitialExtendedState = IntPtr.Zero;
        public SynchronizationContext overlayThreadContext = null;

        private volatile bool _windowActivated;
        /// <summary>
        ...
        public event EventHandler<OverlayWindowsMessage> OnWindowMessage;
        ...
        {            
            if(_windowInitialExtendedState == IntPtr.Zero)
            {
                _windowInitialExtendedState = User32.GetWindowLongPtr(_handle, (int)GWL.GWL_EXSTYLE);
            }
            User32.SetWindowLongPtr(_handle, (int)GWL.GWL_EXSTYLE, (IntPtr)((ExtendedWindowStyle)_windowInitialExtendedState & ~(ExtendedWindowStyle.Layered)));
        }
        ...
        public void DeActivateWindow()
        {
            ...
            User32.SetWindowLongPtr(_handle, (int)GWL.GWL_EXSTYLE, _windowInitialExtendedState);            
        }
        ...
        {
            overlayThreadContext = SynchronizationContext.Current;
            ...
            while (true)
            {
                ...
                if (User32.PeekMessage(ref message, _handle, 0, 0, 0x0001))
                {
                    switch (message.Msg)
                    {
        ...
        private void SetupWindow()
        {
            ...
            IntPtr cursor = User32.LoadCursor(IntPtr.Zero, (int)IDC_STANDARD_CURSORS.IDC_ARROW);
            ...
            if (_isAppWindow) extendedWindowStyle |= ExtendedWindowStyle.AppWindow;
        ...
        {
            OnWindowMessage?.Invoke(this, new OverlayWindowsMessage(msg, wParam, lParam));
            switch (msg)
            {
